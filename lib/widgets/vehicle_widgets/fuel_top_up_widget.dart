...
class FuelTopUpWidget extends StatefulWidget {
  const FuelTopUpWidget({Key? key, required this.vehicle}) : super(key: key);
  ...
  _submit() async {
    ...
    try {
      ...
      var ft = FuelTopUp(
          ...
          position: pos);
      var res = await dataApiDog.addFuelTopUp(ft);
      pp('$mm ... re: ${res.toJson()}');
      showClose = true;
    } catch (e, s) {
      pp('$e $s');
...
class BrandSelector extends StatelessWidget {
  const BrandSelector(
      {super.key, required this.fuelBrands, required this.onPicked});
